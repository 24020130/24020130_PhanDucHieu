...
bool ShowMenu(SDL_Renderer* g_screen)
{
    ...
    // l?y kích th??c màn hình
    const int SCREEN_WIDTH = 1280;
    const int SCREEN_HEIGHT = 640;

    // Xác ??nh v? trí nut (C?n vào gi?a màn hình)
    SDL_Rect startRect;
    startRect.w = 200;  // Chi?u r?ng c?a nút
    startRect.h = 80;   // chi?u cai c?a nút
    ...
    while (true)
    {
        ...
        // V? menu background và nút Start
        SDL_RenderClear(g_screen);
...
bool InitData()
{
    ...
    else
    {
        ...
        // Ki?m tra xem  âm thanh có thành công không
...
// B? sung ph??ng th?c gi?i phóng các tài nguyên âm thanh trong hàm close()
void close()
{
    g_background.Free();
    ...
    while (game_running)
    {
        ...
        while (!quit)
        {
                    ...
                    for (int am = 0; am < bullet_list.size(); am++)
                    {
                        ...
                        if (p_bullet)
                        {
                            ...
                            if (is_col1 == true)
                            {
                                ...
                                // Phát âm thanh khi ??n threat trúng ng??i ch?i
#ifdef USE_AUDIO 
            ...
            for (int am = 0; am < bullet_arr.size(); am++)
            {
                ...
                if (p_bullet)
                {
                    // X? lý va ch?m c?a player v?i threats
                    for (int i = 0; i < threats_list.size(); i++)
                    {
                        ThreatsObject* obj_threat = threats_list.at(i);
                        if (obj_threat != NULL)
                    ...
                    if (bossObject.GetHealth() > 0) {  // ch? ki?m tra n?u boss còn s?ng
                        ...
                        if (b_col1)
                        {
                            ...
                            Mix_PlayChannel(-1, g_sound_explosion, 0);
#endif
                            bossObject.DecreaseHealth(); // Gi?m máu c?a boss
                            for (int ex = 0; ex < 8; ex++)
                            {
                                int x_pos = p_bullet->GetRect().x - frame_exp_width * 0.5;
                                int y_pos = p_bullet->GetRect().y - frame_exp_height * 0.5;
                            ...
                            if (bossObject.GetHealth() <= 0) {   // Boss ch?t
                                ...
                                // Phát âm thanh boss b? tiêu di?t
#ifdef USE_AUDIO
            ...
            // Sau khi tiêu di?t boss, hi?nn th? thông báo chi?n th?ng
...
#ifdef USE_AUDIO
                    Mix_HaltChannel(-1); // Dùng t?t c?  các kênh âm thanh
#endif
            ...
            if (val <= SCREEN_WIDTH)
            {
                // Phát âm thanh khi boss xu?t hi?n l?n sau
                if (!boss_appeared)
                {
                    boss_appeared = true;
#ifdef USE_AUDIO
                    // gi?m âm l??ng c?a nh?c
                    Mix_Volume(-1, MIX_MAX_VOLUME / 2);
#endif
                }

                // In ra v? trí c?a boss
                ...
                // X? lý va ch?m boss v?i ng??i ch?i
                SDL_Rect rect_player = p_player.GetRectFrame();
                ...
                // X? lý va ch?m gi?a player và boss
                if (bossObject.GetHealth() > 0) {  // Ch? ki?m tra n?u boss còn s?ng
                    SDL_Rect boss_rect = bossObject.GetRectFrame();
        ...
        if (restart_to_menu) {
            // Dùng t?t c? âm thanh tr??cc khi hi?n th? màn hình game over
#ifdef USE_AUDIO
            Mix_HaltChannel(-1); 
#endif
